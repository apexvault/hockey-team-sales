...
| ~~F-36~~ | LOW | Cart-existence and basket-total oracles reachable with only the public publishable key, via **three** routes that all call `listShippingOptionsForCartWorkflow` (or equivalent) on a caller-supplied `cart_id`: `GET /store/free-shipping/prices`, `GET /store/shipping-options` (whose 404 echoed the probed id back), and `POST /store/shipping-options/:id/calculate`. | **FIXED** — all three guarded; real vs unknown now return an identical 403. `?cart_id=` on `/store/products` and `/store/product-variants` was measured and is **not** an oracle (real and unknown both return an empty list). |
| F-43 | **MEDIUM** | **`POST /store/payment-collections/:id/payment-sessions` is unguarded** — with a collection id, an unauthenticated caller can read the victim's basket `amount` **and create payment sessions on their collection**, each carrying `context.customer` = the attacker. It **writes**, so it is a cost and abuse vector against a real PSP independent of the leak, and a payment-confusion risk when a real provider is wired. Deferrable only because no acquisition path for an owned cart's collection id was found — `GET /store/carts/:id`, `?fields=+payment_collection.id` and `POST /store/payment-collections` are all now 403, there is no `GET /store/payment-collections/:id` route, and `pay_col_<ULID>` is not guessable. The remaining route to an id is a **guest** cart, i.e. F-42. Fix is one `cart_payment_collection` link hop, the same two-hop shape `ensureApprovalAccess` already uses. | **OPEN** — gated with F-42 |
| F-42 | **MEDIUM** | **Guest carts containing PII can be read, claimed, and their contents COPIED OUT PERMANENTLY by anyone holding the cart id.** The `has_account === false` branch of `ensureCartNotOwnedByAnother` does not cover "unclaimed" carts (those have no customer at all) — Medusa attaches a guest customer the moment an email is entered, so the branch's real scope is guest carts that **already contain** email, shipping address and, for this product, a player's name and number. The branch is load-bearing: removing it refuses every guest shopper on their own cart. Medusa has no session-bound cart, so for a guest the cart id genuinely is the only credential. **Blast radius is copy-out-and-keep, not merely read** (measured): a stranger can `POST /store/quotes` with a guest cart id and receive a permanent, attacker-owned draft order containing the basket — and the real parent later registering and claiming the cart does **not** revoke it. They can also create a payment collection on it and read its total via free-shipping. Closing it needs a server-set cart nonce, or requiring a claimant's authenticated email to match `cart.email`. | **OPEN — accepted residual**, tracked under P0-SEC-7. Must be closed before real guest traffic carrying minors' data; "before" has to account for copy-out, not just read. |
